...
>(({ className, children, style, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex h-10 w-full items-center justify-between rounded-md border px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-1 disabled:cursor-not-allowed disabled:opacity-50',
      className
    )}
    style={{
...
>(({ className, children, style, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      '[&[data-state=checked]]:bg-[color:var(--background-color-brand-primary)] [&[data-state=checked]]:text-[color:var(--text-color-brand-primary)]',
      className
    )}
    style={style}
    {...props}
