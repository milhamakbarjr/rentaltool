...
import { RENTAL_STATUSES, RENTAL_STATUS_COLORS, ROUTES } from "@/utils/constants";
...
export const RentalsMain = () => {
    ...
    const filters = useMemo<RentalFilterData>(() => {
        ...
        if (selectedTab !== "all") {
            // Only assign if selectedTab is a valid status
            const allowedStatuses: RentalFilterData["status"][] = [
                "draft",
                "upcoming",
                "active",
                "completed",
            ...
            if (allowedStatuses.includes(selectedTab as RentalFilterData["status"])) {
                baseFilters.status = selectedTab as RentalFilterData["status"];
            }
        }
    ...
    const sortedRentals = useMemo(() => {
        ...
        return [...rentals].sort((a, b) => {
            ...
            if (sortDescriptor.column === "customer" && typeof first === "object" && typeof second === "object") {
                first = (first as { name?: string })?.name;
                second = (second as { name?: string })?.name;
            }
    ...
    const totalPages = useMemo(() => {
        ...
        return Math.max(1, Math.ceil(rentals.length / pageSize));
    }, [rentals]);
        ...
        <div className="bg-primary">
            <main className="bg-primary pt-8 pb-12 lg:pt-12 lg:pb-24">
                <div className="flex flex-col gap-8">
                    <div className="mx-auto flex w-full max-w-container flex-col gap-5 px-4 lg:px-8">
                        ...
                        <div className="relative flex flex-col gap-5 bg-primary">
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                <div>
                                    <h1 className="text-display-xs font-semibold text-primary lg:text-display-sm">{t("title")}</h1>
                        ...
                        <div className="flex justify-between gap-6">
                            <Tabs selectedKey={selectedTab} onSelectionChange={(value) => setSelectedTab(value as string)} className="w-auto">
                                <TabList
                                    type="button-minimal"
                                    items={[
                                        { id: "all", label: t("allRentals") },
                                        { id: RENTAL_STATUSES.DRAFT, label: t("draft") },
                                        { id: RENTAL_STATUSES.ACTIVE, label: t("active") },
                                        { id: RENTAL_STATUSES.UPCOMING, label: t("upcoming") },
                                        { id: RENTAL_STATUSES.COMPLETED, label: t("completed") },
                                        { id: RENTAL_STATUSES.OVERDUE, label: t("overdue") },
                                        { id: RENTAL_STATUSES.CANCELLED, label: t("cancelled") },
                                    ]}
                                />
                            </Tabs>
                        </div>
                    ...
                    {/* Metrics Cards */}
                    {/* TODO: Replace placeholder percentage values with actual historical data comparison */}
                    <div className="mx-auto flex w-full max-w-container flex-col gap-5 px-4 md:flex-row md:flex-wrap lg:gap-6 lg:px-8">
                        ...
                        <MetricsChart04
                            ...
                            subtitle={t("activeRentalsMetric")}
                            className="flex-1 md:min-w-[280px]"
                            type="simple"
                            change="0%"
                            changeTrend="positive"
                    ...
                    <div className="mx-auto flex w-full max-w-container flex-col gap-6 px-4 lg:px-8">
                        <TableCard.Root className="bg-secondary_subtle shadow-xs lg:rounded-xl">
                            <div className="flex gap-4 px-5 pt-3 pb-2.5">
                                <p className="text-sm font-semibold text-primary">{t("allRentals")}</p>
                            </div>
                            ...
                            {error ? (
                                <div className="flex flex-col items-center justify-center bg-primary px-5 py-12">
                                    <p className="text-sm text-error">{t("errorLoading")}</p>
                                </div>
                            ) : isLoading ? (
                                <div
                                    className="flex flex-col items-center justify-center bg-primary px-5 py-12"
                                    role="status"
                                    aria-live="polite"
                                >
                                    <span className="sr-only">{t("loadingRentals")}</span>
                                ...
                                <>
                                    ...
                                    >
                                        <Table.Header className="bg-primary">
                                            <Table.Head id="rental_number" label={t("rentalNumber")} allowsSorting isRowHeader className="w-full" />
                                            <Table.Head id="customer" label={t("customer")} allowsSorting className="max-md:hidden" />
                                            <Table.Head id="start_date" label={t("startDate")} allowsSorting />
                                            <Table.Head id="end_date" label={t("endDate")} allowsSorting className="max-lg:hidden" />
                                            <Table.Head id="status" label={t("status")} allowsSorting />
                                            <Table.Head id="total_amount" label={t("totalAmount")} allowsSorting className="max-md:hidden" />
                                        </Table.Header>
                                            ...
                                            {(rental) => (
                                                <Table.Row id={rental.id}>
                                                    ...
                                                    <Table.Cell>
                                                        <BadgeWithDot
                                                            color={
                                                                (RENTAL_STATUS_COLORS[rental.status] as
                                                                    | "success"
                                                                    | "gray"
                                                                    | "error"
                                                                    | "blue"
                                    ...
                                    <PaginationCardMinimal align="right" page={1} total={totalPages} className="bg-primary" />
                                </>
                            ) : (
                                <div className="flex flex-col items-center justify-center bg-primary px-5 py-12">
                                    <p className="text-sm text-tertiary">{t("noRentalsFound")}</p>
                                </div>
                            )}
                        </TableCard.Root>
                    </div>
